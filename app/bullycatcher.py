...
class Bullycatcher:
  ...
  SYSTEM_PROMPT = """
     ...
     Make sure to consider the messages before as the context may rule out the comment being meant in a bullying way. If you sense that the context does rule out the comment as bullying then please ONLY return "False" and DONT return an explanation.
     Try not to repeat the same message twice.
     Make sure to understand that after a user has said something bullying you should not tell them again and aim to instead move on from that original comment by returning "False" at other non-bullying comments without giving an explanation.
  """
